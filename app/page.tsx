      ...
      <section className="relative min-h-[90vh] flex items-center justify-center px-4 py-12 overflow-hidden">
        ...
        <div className="max-w-7xl mx-auto relative z-10">
          ...
          >
            ...
            >
              <motion.div 
                className="relative bg-gradient-to-br from-card to-card/50 border rounded-3xl overflow-hidden shadow-2xl backdrop-blur-sm always-glow"
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8 max-w-6xl mx-auto">
            {solutions.map((solution, index) => (
              <motion.div
                ...
                className="group relative always-glow"
              >
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 max-w-6xl mx-auto">
            {testimonials.map((testimonial, index) => (
              <motion.div
                ...
                animate={{ opacity: 1, y: 0 }}
                transition={{ duration: 0.6, delay: index * 0.1 }}
                className="bg-card/50 backdrop-blur-sm p-8 rounded-2xl border border-border/50 shadow-lg hover:shadow-xl transition-all duration-300 group relative overflow-hidden always-glow"
                whileHover={{ scale: 1.02 }}
              >
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8 max-w-6xl mx-auto">
            {pricingPlans.map((plan, index) => (
              <motion.div
                ...
                viewport={{ once: true, margin: "-100px" }}
                transition={{ duration: 0.6, delay: index * 0.1 }}
                className={`relative bg-card/50 backdrop-blur-sm p-8 rounded-2xl border shadow-lg hover:shadow-xl transition-all duration-300 group always-glow ${
                  plan.popular ? 'border-primary/50 scale-[1.02]' : 'border-border/50'
                }`}
                whileHover={{ y: -10 }}
              >
                <motion.div
                  className={`pricing-card${plan.popular ? ' border-purple-500' : ''}`}
                  whileHover={{ y: -10 }}
                >
                  {plan.popular && (
